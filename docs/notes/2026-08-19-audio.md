...
> **WRONG, and corrected on 2026-08-29.** The list below is the contract this
> file published, and **no producer ever honoured it**. `combat/engine.js`
> forwards every engine event to the bus as `combat:` + the type, so the bus
> carried `combat:damage` while `audio.js` waited on `damage`; two `EV` entries
> are themselves named `combat:start` and `combat:end`, so what actually went
> past was `combat:combat:start`. 33 of the 38 subscriptions were dead from the
> day they were written, and the payloads never matched either — `_hpTension`
> reads `p.hp` / `p.maxHp`, and no engine event carries those fields.
>
> The game was not silent: `scenes/combat.js` plays thirteen of these cues
> DIRECTLY, timed to its FX, which is where a hit sound belongs. The dead block
> is gone. **Do not re-wire these names onto the bus** — that would stack a
> second voice on all thirteen. A new sound gets a call at the moment it
> happens, like everything else in `scenes/`.
>
> `tests/bus-names/check.py` resolves this file's `on(...)` alias now and fails
> on any subscription nothing emits, so this cannot come back.
>
> The list is kept verbatim because it is the evidence: a published contract
> with no producer reads exactly like a working one.
