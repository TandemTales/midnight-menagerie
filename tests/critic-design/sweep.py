...
ap.add_argument("--out", default="sweep-result.json")
...
URL = (f"http://localhost:8777/tests/critic-design/sweep.html?tier={a.tier}&enc={a.enc}"
       f"&region={a.region}&n={a.n}&gen={a.gen}&bots={a.bots}&pol={a.pol}"
       f"&scales={a.scales}&seed={a.seed}&heal={a.heal}&haunt={a.haunt}")
...
print(f"== sweep {a.region}/{a.tier} {a.enc or '(rolled)'}  haunt {a.haunt}  loadouts {R['loadouts']}")
