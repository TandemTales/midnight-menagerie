...
/**
 * Mr. Moth himself, the one hero of this board: a tall, soft, dusty moth in a
 * shopkeeper's coat with a ruff of fur at his throat and his wings folded down
 * his back, holding low in front of him the lamp that keeps the market
 * findable — so he is lit from below, warm against the dark shop behind him,
 * and the lamp is in his big glossy eyes the way it is in every creature on
 * UI/selectCompanion.png.
 *
 * Round 6: he is a PAINTING (moth-portrait.webp, tools/prep_ui_focal.py, from
 * SABLE's round 5 board), standing at the size of the hero of a picture inside
 * the tall arched niche of the Kid board's own mirror (moth-mirror.webp) at the
 * head of his shelf. He used to be line-work on enamel in a narrow niche, which
 * left the row of Tricks without a host.
 */
const MOTH_ART = `
<span class="sh-moth" role="img" aria-label="Mr. Moth, behind his counter"></span>`;
...
export class ShopScene extends RoomScene {
  ...
  _layout() {
    // Mr. Moth's niche is as tall as his cabinet is, whatever the board gave
    // it: the cabinet's height is the grid's to decide, so it is measured and
    // handed to shop.css, which derives the niche — and the head of the shelf
    // the Tricks start after — from it.
    const cab = this.root?.querySelector('.sh-counter--cards');
    if (cab?.clientHeight) cab.style.setProperty('--cab-h', `${cab.clientHeight}px`);
