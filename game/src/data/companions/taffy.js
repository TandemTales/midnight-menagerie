...
const commons = [
  ...
  },
  {
    ...
    cost: 1, target: SELF, keywords: ['split'],
    text: '[Split] {n}. Gain {b} Guard.',
    flavor: 'A modest donation to the floor.',
    nums: { n: 1, b: 6 },
    effect: eff(c => { split(c, N(c).n); U.guard(c, N(c).b); }),
  ...
  },
  {
    ...
    cost: 1, target: NONE, keywords: ['stretch'],
    text: '[Stretch] another Attack or Skill.',
    flavor: 'Patience, applied physically.',
    nums: {},
    effect: eff(async c => { const [k] = await U.pickCards(c, { pile: 'hand', count: 1, prompt: 'Stretch a Trick', filter: copyable }); stretch(c, k); }),
    upgrade: { cost: 0 },
  },
  {
